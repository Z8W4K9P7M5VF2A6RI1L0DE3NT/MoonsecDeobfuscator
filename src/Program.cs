...
namespace GalacticDecompilerBot
{
    public static class Program
    {
        ...
        private static async Task<string> WebsocketDecompile(string bytecodePath)
        {
            ...
            client.MessageReceived.Subscribe(msg =>
            {
                ...
                else if (msg.MessageType == WebSocketMessageType.Binary)
                {
                    try
                    {
                        ...
                        Console.WriteLine("[WS-BIN] " + txt);

                        luaResult += txt;
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("[DECRYPT ERROR] " + ex);
                    }
                }
            });
            ...
            if (!serverReady)
            {
                Console.WriteLine("Websocket never connected.");
                return "";
            }
    ...
    public static class LuaRenamer
    {
        ...
        private static string IsolateFunctions(string src) => src;

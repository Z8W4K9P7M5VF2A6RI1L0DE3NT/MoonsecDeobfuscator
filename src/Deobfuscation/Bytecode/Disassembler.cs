...
// Tracks metadata about register values
private class RegisterInfo
{
    public object Value;
    public string Type; // "service", "ui_element", "function", "string", "number"
    public string SuggestedName;
    public bool IsMethodTarget = false; // Set by SELF opcode
}
...
public class ProxyFactory
{
    ...
    private readonly HashSet<string> _uiLibraries = new() { "Library", "Window", "Tab" };
    ...
    public string GetVarName(object obj, string suggestedName = null, string typeHint = null)
    {
        if (_state.Registry.TryGetValue(obj, out var name)) return name;
        
        // Determine base name
        string baseName = suggestedName ?? typeHint ?? "obj";
        ...
        // UI elements get semantic names
...
public class Disassembler
{
    ...
    private FunctionNode BuildFunctionNode(Function function, string name, bool isAnon)
    {
        ...
        for (int i = 0; i < insts.Count; i++)
        {
            ...
            switch (ins.OpCode)
            {
                ...
                case OpCode.LoadK:
                    var constVal = LoadConstant(function, ins.B);
                    regs[ins.A] = new RegisterInfo { 
                        Value = constVal,
                        Type = constVal?.GetType().Name.ToLower() ?? "nil"
                    };
                    break;

                case OpCode.GetTable:
                    HandleTableAccess(regs, ins, function, statements);
                    break;
                ...
                case OpCode.Self:
                    HandleSelfCall(regs, ins, function);
                    break;
                ...
                case OpCode.Closure:
                    // Store as function placeholder - will build actual body later
                    regs[ins.A] = new RegisterInfo { 
                        Value = $"<function_{ins.B}>", 
                        Type = "function",
                        SuggestedName = "callback"
                    };
                    break;
                ...
                case OpCode.Return:
                    if (ins.B > 1)
                    {
                        var retVal = GetRegister(regs, ins.A);
                        statements.Add(new RawNode($"return {FormatValue(retVal)}"));
                    }
                    break;
            }
        }
    ...
    private void HandleTableAccess(Dictionary<int, RegisterInfo> regs, Instruction ins, Function f, List<AstNode> statements)
    {
        var tblInfo = GetRegister(regs, ins.B);
        var keyInfo = GetRk(regs, f, ins.C);
        
        if (tblInfo?.Value is "game" && keyInfo?.Value is string keyStr && keyStr.EndsWith("Service"))
        {
            string serviceVar = keyStr.Replace("Service", "");
            
            // Create proper assignment statement
            var assignNode = new AssignNode(
                serviceVar, 
                $"game:GetService(\"{keyStr}\")", 
                true,
                $"Service: {keyStr}"
            );
            ...
            // Store the variable name in register
            regs[ins.A] = new RegisterInfo { 
                Value = serviceVar, 
                Type = "service",
                SuggestedName = serviceVar
            };
        }
        else
        {
            string access = $"{FormatValue(tblInfo)}.{FormatValue(keyInfo)}";
            regs[ins.A] = new RegisterInfo { 
                Value = access,
                Type = "property"
            };
    ...
    private void HandlePropertyAssignment(Dictionary<int, RegisterInfo> regs, Instruction ins, Function f, List<AstNode> statements)
    {
        var objInfo = GetRegister(regs, ins.A);
        var keyInfo = GetRk(regs, f, ins.B);
        var valInfo = GetRk(regs, f, ins.C);
        
        if (objInfo != null && keyInfo != null)
        {
            statements.Add(new AssignNode(
                $"{FormatValue(objInfo)}.{FormatValue(keyInfo)}", 
                FormatValue(valInfo), 
                false
            ));
        }
    ...
    private void HandleSelfCall(Dictionary<int, RegisterInfo> regs, Instruction ins, Function f)
    {
        var objInfo = GetRegister(regs, ins.B);
        var methodInfo = GetRk(regs, f, ins.C);
        
        if (objInfo != null && methodInfo?.Value is string methodName)
        {
            // Mark as method call and store object for later use
            regs[ins.A] = new RegisterInfo { 
                Value = $"{FormatValue(objInfo)}:{methodName}",
                Type = "method",
                IsMethodTarget = true
            };
            // Store self reference in next register
            regs[ins.A + 1] = objInfo;
        }
    }

    private void HandleCall(Dictionary<int, RegisterInfo> regs, Instruction ins, Function f, List<AstNode> statements)
    {
        var funcInfo = GetRegister(regs, ins.A);
        if (funcInfo == null) return;

        string funcStr = FormatValue(funcInfo);
        bool isMethodCall = funcInfo.IsMethodTarget || funcStr.Contains(':');
        
        // Determine argument start register
        // For method calls: args start at A+2 (skip self at A+1)
        // For regular calls: args start at A+1
        int argStart = isMethodCall ? ins.A + 2 : ins.A + 1;
        int argCount = Math.Max(0, ins.B - (isMethodCall ? 2 : 1));
        
        var args = Enumerable.Range(argStart, argCount)
                           .Select(r => GetRegister(regs, r))
                           .Where(r => r != null)
                           .Select(r => FormatValue(r))
                           .ToList();

        // Special handling for UI library loadstring pattern
        if (funcStr == "loadstring" && args.Count >= 1)
        {
            var firstArg = args[0];
            var httpMatch = Regex.Match(firstArg, @"game:HttpGet\(""([^""]+)""\)");
            if (httpMatch.Success)
            {
                string url = httpMatch.Groups[1].Value;
                _state.LastHttpUrl = url;
                _state.StringRefs.Add(new StringRef { 
                    Value = url, 
                    Hint = "HTTP URL",
        ...
        // Handle UI method calls
        if (funcStr.StartsWith("Library:") || funcStr.StartsWith("Window:") || funcStr.StartsWith("Tab:"))
        {
            string resultVar = null;
            if (funcStr.Contains("CreateWindow")) resultVar = "Window";
            else if (funcStr.Contains("CreateTab")) resultVar = "Tab";
            else if (funcStr.Contains("CreateSection")) resultVar = "Section";
            
            if (resultVar != null)
            {
                var varName = _proxyFactory.GetVarName(funcInfo, resultVar, "ui_element");
                statements.Add(new AssignNode(
                    varName, 
                    $"{funcStr}({string.Join(", ", args)})", 
                    true,
                    $"UI: {resultVar}"
                ));
                regs[ins.A] = new RegisterInfo { 
            ...
            {
                statements.Add(new CallNode(funcStr, args, true));
            }
            return;
        }
        ...
        statements.Add(new CallNode(funcStr, args, isMethodCall));

        // Handle return value
        if (ins.C - 1 > 0)
        {
            string varName = _proxyFactory.GetVarName(funcInfo, "result", "result");
            regs[ins.A] = new RegisterInfo { 
                Value = varName,
                Type = "result",
                SuggestedName = varName
    ...
    private string FormatNode(AstNode node, int indent)
    {
        ...
        return node switch
        {
            ...
            FunctionNode fn => $"{prefix}function {fn.Name}({string.Join(", ", fn.Parameters)})\n{FormatBlock(fn.Body, indent + 1)}\n{prefix}end",
            _ => ""
        };
    }

    private string FormatBlock(Block block, int indent)
    {
        var sb = new StringBuilder();
        foreach (var stmt in block.Statements)
        {
            sb.AppendLine(FormatNode(stmt, indent));
        }
        return sb.ToString().TrimEnd();
    }

    private RegisterInfo GetRegister(Dictionary<int, RegisterInfo> regs, int r) => 
        regs.TryGetValue(r, out var v) ? v : null;

    private RegisterInfo GetRk(Dictionary<int, RegisterInfo> regs, Function f, int val) => 
        val >= 256 ? new RegisterInfo { Value = LoadConstant(f, val - 256) } : GetRegister(regs, val);

    private string FormatValue(RegisterInfo info)
    {
        if (info == null) return "nil";
        if (info.Value is string s) return $"\"{s}\"";
        if (info.Value is bool b) return b.ToString().ToLower();
        if (info.Value is double d) return d.ToString();
        if (info.Value is int i) return i.ToString();
        return info.Value?.ToString() ?? "nil";
    }
...
// --- Lua Dumper Deobfuscation Step Integration ---
// The following region contains the Lua dumper logic provided by the user,
// which can be executed as a post-processing step for complex obfuscation patterns.
// This integration allows the C# deobfuscator to invoke the Lua dumper when
// static analysis is insufficient.

public static class LuaDumperStep
{
    // Embedded Lua dumper script as a string resource
    // This is the complete Lua code provided by the user, formatted as a C# string
    public const string LuaDumperScript = @"
-- Lua Dumper Deobfuscation Step
-- ixvixv4 simulation engine implementation
-- This script is executed for complex runtime-based obfuscation patterns
...
local function aW(x, aQ, aX, aS)
    ...
    if aY and aY:match(""^ixvixv4%d+$"") then
        return aY
    end
    t.ixvixv4_counter = (t.ixvixv4_counter or 0) + 1
    local am = ""ixvixv4"" .. t.ixvixv4_counter
...
loadstring = function(al, eu)
    ...
    if cI:match(""^https?://"") then
        ...
        at(string.format('local ixvixv4sixseven = loadstring(game:HttpGet(""%s""))()', cI))
        return function()
            return ez
        end
    end
...
function q.dump_file(eN, eO)
    q.reset()
    az(""this file is generated using ixvixv4"")
    ...
    local eR =
        ...
        {LuraphContinue = function()
            end, script = script, game = game, workspace = workspace, IXVIXV4_CHECKINDEX = function(x, ba)
                local aF = x[ba]
                if j(aF) == ""table"" and not t.registry[aF] then
                    t.ixvixv4_counter = t.ixvixv4_counter + 1
                    t.registry[aF] = ""ixvixv4tab"" .. t.ixvixv4_counter
                end
                return aF
            end, IXVIXV4_GET = function(b5)
                return b5
            end, IXVIXV4_CALL = function(as, ...)
                return as(...)
            end, IXVIXV4_NAMECALL = function(eS, em, ...)
                return eS[em](eS, ...)
...
";
    ...
    // Integration method to execute the Lua dumper
    public static void ExecuteLuaDumper(string inputFile, string outputFile = null)
    {
        // This method would be called from the main deobfuscator
        // when additional runtime analysis is needed
        var lua = new Lua();
        lua.DoString(LuaDumperScript);
        dynamic dumper = lua["q"];
        dumper.dump_file(inputFile, outputFile ?? "lua_dumper_output.lua");
    }
}

// Extension method to integrate Lua dumper step into the main deobfuscation pipeline
public static class DeobfuscatorPipelineExtensions
{
    public static string AddLuaDumperStep(this Disassembler disassembler, string intermediateLua)
    {
        // When static analysis is insufficient, run the Lua dumper
        // This creates a hybrid approach: bytecode analysis + runtime simulation
        
        var tempFile = Path.GetTempFileName() + ".lua";
        File.WriteAllText(tempFile, intermediateLua);
        
        try
        {
            LuaDumperStep.ExecuteLuaDumper(tempFile, "enhanced_output.lua");
            
            if (File.Exists("enhanced_output.lua"))
            {
                var enhancedOutput = File.ReadAllText("enhanced_output.lua");
                // Merge with existing deobfuscation results
                return enhancedOutput;
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Lua dumper step failed: {ex.Message}");
        }
        finally
        {
